#!/usr/bin/env python3
"""
Backtesting pro strategii SMA-14 + Fear & Greed.
...
Spuštění: python sma_backtest.py
Výstup:   výsledky na konzoli + sma_backtest_results.json
"""
...
def get_sell_fraction(fear: int, base_balance: float) -> float:
    """Vrátí množství mince k prodeji — přesná kopie logiky z main.py."""
...
def backtest_coin(df: pd.DataFrame, fear_greed: dict, symbol: str) -> dict:
    """Provede backtesting SMA strategie pro jednu minci."""
    ...
    for i in range(SMA_PERIOD, len(closes)):
        ...
        # SMA přes posledních HOURS_BACK svíček (max SMA_PERIOD)
        window = closes[max(0, i - HOURS_BACK + 1): i + 1]
        sma = float(np.mean(window[-SMA_PERIOD:]))
        trend_up = price > sma

        # Signál — přesná logika z main.py
        signal = "HOLD"
...
def print_results(results: dict):
    width = 100
    print("\n" + "=" * width)
    print(f"  BACKTEST VÝSLEDKY — SMA-{SMA_PERIOD} + Fear & Greed")
    ...
    for category, symbols in COINS.items():
        ...
        for symbol in symbols:
            ...
            try:
                ...
                # odebrat seznam obchodů z výsledků pro tisk, uložit zvlášť
