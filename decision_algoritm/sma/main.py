...
def evaluate_market(client, symbol):
    prices = get_price_data(client, symbol, config["trading"]["hours_back"])
...
def run_cycle(symbol, base_asset, quote_asset):
    ...
    try:
        ...
        save_decision(signal, symbol, 0.0, 0.0, None, 0.0, position_size, f"Initialization failed: {exc}")
        return "HOLD"
...
def main():
    ...
    setup_logging(config["logging"], symbol, console_output=True)
