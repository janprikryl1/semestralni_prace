...
def get_quantity_precision(step_size: str) -> int:
    """Odvodí počet desetinných míst ze stepSize Binance ('1' → 0, '0.001' → 3)."""
    exp = Decimal(step_size).as_tuple().exponent
    return max(0, -exp)


def _compute_requirements(filters, price: float) -> dict:
    """Stejná logika jako get_buy_order_requirements, ale používá předem načtené filtry."""
...
def _adjust_and_format(filters, quantity: float) -> tuple:
    """Zaokrouhlí quantity na LOT_SIZE a vrátí (float, string) se správnou přesností."""
    lot_filter = get_lot_size_filter(filters)
    if not lot_filter:
